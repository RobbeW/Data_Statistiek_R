...
\begin{document}
	\begin{tikzpicture}[xscale=0.5, font=\small, tl/.style = {black, inner sep=1pt, font=\scriptsize} ]
		...
		% axes
		\draw[main, ->,very thick] (-13,0) -- (13,0) node[above right] {$x$};
		\draw[main, ->,very thick] (0,-3.8) -- (0, 2.8) node[above left] {$y$};
		% curve
		\draw[yscale=0.33,thick,PineGreen, draw=PineGreen,	domain=-12.5:12.5,samples=300,variable=\x] plot(\x,{sin(deg{\x})*\x}) node [below right] {$y=f(x)$};
	\end{tikzpicture}
\end{document}
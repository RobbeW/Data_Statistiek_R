...
\begin{document}
	\begin{tikzpicture}[main, line join=bevel]
		...
		\node[thick, main] (B) [below right of=A]  {\parbox{1.75cm}{\centering halte \texttt{1}}}; 
		\node[thick, main] (C) [below left of=B] {\parbox{1.75cm}{\centering halte \texttt{2}}}; 
		\node[thick, main] (D) [above left of=C] {\parbox{1.75cm}{\centering halte \texttt{3}\\ eindhalte}}; 
		
		\draw [thick, main] (A) edge[bend left, track] node [above right, xshift=0.25cm, yshift=0.25cm] {\textbf{1}} (B); 
		...
		\draw [thick, main, BurntOrange] (D) edge[bend left, track] node [above left, xshift=-0.25cm, yshift=0.25cm] {\textbf{4}} (A); 
	\end{tikzpicture}
\end{document}

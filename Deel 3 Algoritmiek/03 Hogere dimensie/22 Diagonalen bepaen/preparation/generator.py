...
# generate test data
ntests = 20
cases = [[[1, 2, 3], [4, 5, 6], [7, 8, 9]], [[1, 2], [3, 4]]]

for n in range(20):
    r = random.randint(1,20)
    mat = []
    for _ in range(r):
        rij = [ random.randint(-10,10) for _ in range(r)]
        mat.append(rij)
...
for i in range(len(cases)):
    ...
    expression_name = f"diagonalen({test})"
    description_name = generate_expression("diagonalen", test)
    description = {"description": description_name, "format": "python"}

    try:
        outputF = io.StringIO()
        with contextlib.redirect_stdout(outputF):
            result = module.diagonalen(test)

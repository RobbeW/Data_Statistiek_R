...
# generate test data
ntests = 20
cases = [([[1, 0, -3], [4, -2, 5]], 2), ([[1, 2, 3], [4, 5, 6], [7, 8, 9]], 0)]
...
for i in range(len(cases)):
    ...
    expression_name = f"scalair_product({test[0]}, {test[1]})"
    description_name = generate_expression("scalair_product", test[0], (test[1], ))
    description = {"description": description_name, "format": "python"}

    try:
        outputF = io.StringIO()
        with contextlib.redirect_stdout(outputF):
            result = module.scalair_product(test[0], test[1])

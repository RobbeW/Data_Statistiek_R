...
while len(cases) < ntests:
    ...
    K = random.randint(3, 35)
    
    mat = []
    for r in range(R):
        row = [random.randint(0,99) for _ in range(K)]
        mat.append(row)
...
for i in range(len(cases)):
    ...
    expression_name = f"zandloper({test})"
    description_name = generate_expression("zandloper", test)
    description = {"description": description_name, "format": "python"}

    try:
        outputF = io.StringIO()
        with contextlib.redirect_stdout(outputF):
            result = module.zandloper(test)

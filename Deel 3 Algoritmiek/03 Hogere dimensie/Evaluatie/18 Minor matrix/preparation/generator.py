...
while len(cases) < ntests:
    ...
    mat = []
    for _ in range(R):
        rij = [ random.randint(-5,5) for _ in range(C)]
        mat.append(rij)
    
    r = random.randint(0, R - 1) + 1
    c = random.randint(0, C - 1) + 1
    case = (mat, r, c)
...
for i in range(len(cases)):
    ...
    expression_name = f"minor_matrix({test[0]}, {test[1]}, {test[2]})"
    description_name = generate_expression("minor_matrix", test[0], [test[1], test[2]] )
    description = {"description": description_name, "format": "python"}

    try:
        outputF = io.StringIO()
        with contextlib.redirect_stdout(outputF):
            result = module.minor_matrix(test[0], test[1], test[2])

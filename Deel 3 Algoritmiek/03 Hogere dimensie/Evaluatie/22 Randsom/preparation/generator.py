...
while len(cases) < ntests:
    ...
    mat = []
    for r in range(R):
        rij = [random.randint(-9,9) for _ in range(C)]
        mat.append(rij)
...
for i in range(len(cases)):
    ...
    expression_name = f"randsom({test})"
    description_name = generate_expression("randsom", test)
    description = {"description": description_name, "format": "python"}

    try:
        outputF = io.StringIO()
        with contextlib.redirect_stdout(outputF):
            result = module.randsom(test)

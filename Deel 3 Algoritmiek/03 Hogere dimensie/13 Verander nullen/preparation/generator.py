...
while len(cases) < ntests:
    ...
    matrix = []
    for _ in range(r):
        row =  [random.randint(-15,15) for _ in range(c)]
        matrix.append(row)


    count = 0
    aantal_zero = random.randint(0, r*c)
    for _ in range(aantal_zero):
...
for i in range(len(cases)):
    ...
    expression_name = f"nullen({test})"
    description_name = generate_expression("nullen", test)
    description = {"description": description_name, "format": "python"}

    try:
        outputF = io.StringIO()
        with contextlib.redirect_stdout(outputF):
            result = module.nullen(test)

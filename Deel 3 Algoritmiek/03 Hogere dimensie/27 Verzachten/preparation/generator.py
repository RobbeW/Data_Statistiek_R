...
# generate test data
ntests = 20
cases = [([[1, 2, 3, 5], [4, 5, 6, 7], [7, 8, 9, 0]], 0, 1)]
...
while len(cases) < ntests:
    ...
    mat = []
    for _ in range(r):
        rij = [ random.randint(0,255) for _ in range(c)]
        mat.append(rij)
...
for i in range(len(cases)):
    ...
    expression_name = f"stap({test[0]}, {test[1]}, {test[2]})"
    description_name = generate_expression("stap", test[0], (test[1], test[2]) )
    description = {"description": description_name, "format": "python"}

    try:
        outputF = io.StringIO()
        with contextlib.redirect_stdout(outputF):
            result = module.stap(test[0], test[1], test[2])
...
for i in range(len(cases)):
    ...
    # generate test expression
    expression_name = f"vervagen({test[0]})"
    description_name = generate_expression("vervagen", test[0])
    description = {"description": description_name, "format": "python"}

    try:
        outputF = io.StringIO()
        with contextlib.redirect_stdout(outputF):
            result = module.vervagen(test[0])
        stdout = outputF.getvalue() ##printed value

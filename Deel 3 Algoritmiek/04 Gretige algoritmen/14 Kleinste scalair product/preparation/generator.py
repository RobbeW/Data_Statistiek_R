...
while len(cases) < ntests:
    ...
    a = [random.randint(-5,20) for _ in range(n)]
    b = [random.randint(-5,20) for _ in range(n)]
...
for i in range(len(cases)):
    ...
    expression_name = f"minimaal_scalair_product({test[0]}, {test[1]})"
    ...
    try:
        ...
        with contextlib.redirect_stdout(outputF):
            result = module.minimaal_scalair_product(test[0], test[1])

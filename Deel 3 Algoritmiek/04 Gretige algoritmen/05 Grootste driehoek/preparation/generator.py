...
# generate test data
ntests = 20
cases = [[2, 1, 2], [1, 2, 1, 10], [25, 15, 1, 2, 6, 8]]

while len(cases) < ntests:
    e = random.randint(0,3)
    
    n = random.randint(10**e, 10**(e+1))
    zijden = [random.randint(1,100) for _ in range(n)]
    
    if zijden not in cases:
        cases.append(zijden)
...
for i in range(len(cases)):
    ...
    expression_name = f"grootste_omtrek({test})"
    ...
    try:
        ...
        with contextlib.redirect_stdout(outputF):
            result = module.grootste_omtrek(test)

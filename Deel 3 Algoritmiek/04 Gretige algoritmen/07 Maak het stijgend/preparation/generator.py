...
while len(cases) < ntests:
    ...
    getallen = [random.randint(1,10**(e+1)) for _ in range(n)]
    
    if getallen not in cases:
        cases.append(getallen)
...
for i in range(len(cases)):
    ...
    expression_name = f"minimale_toevoegingen({test})"
    #description_name = generate_expression("grootste_omtrek", test)
    #description = {"description": description_name, "format": "python"}

    try:
        outputF = io.StringIO()
        with contextlib.redirect_stdout(outputF):
            result = module.minimale_toevoegingen(test)

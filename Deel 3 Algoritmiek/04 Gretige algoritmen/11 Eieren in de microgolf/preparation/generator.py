...
# generate test data
ntests = 25
cases = [(2, 120, [50, 60, 80]), 
         (5, 50, [40, 40, 50, 55]), 
         (5, 80, [40, 40, 50, 55]),
         (3, 40, [50, 55, 55, 65, 72])]

while len(cases) < ntests:
    max_aantal = random.randint(1, 20)
    max_massa = random.randint(40, 900)
    
    n = random.randint(1, 20)
    eieren = [random.randint(40, 90) for _ in range(n)]
    
    eieren.sort()
    case = (max_aantal, max_massa, eieren)
...
for i in range(len(cases)):
    ...
    expression_name = f"eieren_koken({test[0]}, {test[1]}, {test[2]})"
    ...
    try:
        ...
        with contextlib.redirect_stdout(outputF):
            result = module.eieren_koken(test[0], test[1], test[2])

...
while len(cases) < ntests:
    ...
    perk = [ random.randint(0, 1) for _ in range(n)]
    
    aantal_0 = 0
    for i in range(1, n-1):
        if perk[i] == perk[i-1] == perk[i+1]:
            aantal_0 += 1
    
    case = (perk, max(1,aantal_0 // 3))
...
for i in range(len(cases)):
    ...
    expression_name = f"bloemenperk({test[0]}, {test[1]})"
    ...
    try:
        ...
        with contextlib.redirect_stdout(outputF):
            result = module.bloemenperk(test[0], test[1])

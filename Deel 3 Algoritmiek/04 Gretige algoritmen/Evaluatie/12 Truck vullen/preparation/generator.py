...
while len(cases) < ntests:
    ...
    lijst = [(random.randint(1, 20), random.randint(1, 20)) for _ in range(n)]
    lijst = sorted(lijst, key = lambda x: (-x[1], x[0]))

    total_boxes = sum([lijst[i][1] for i in range(len(lijst))] )
    ...
    maxi += random.randint(0, lijst[k+1][0])

    if random.randint(0,3) == 0:
        maxi = total_boxes + random.randint(-10,10)
...
for i in range(len(cases)):
    ...
    expression_name = f"truck_laden({test[0]}, {test[1]})"
    ...
    try:
        ...
        with contextlib.redirect_stdout(outputF):
            result = module.truck_laden(test[0], test[1])

...
while len(cases) < ntests:
    ...
    for i in range(1, n + 1):
        if random.randint(0,10) != 0:
            num = random.randint(math.ceil(i/1.5), i)
        else:
            num = random.randint(0, i)
        kannen.append(num)
    
    if random.randint(0,1) == 0: # make boom!
        i = random.randint(0, n - 1)
        kannen[i] += random.randint(i//2, n)
        
    
    random.shuffle(kannen)

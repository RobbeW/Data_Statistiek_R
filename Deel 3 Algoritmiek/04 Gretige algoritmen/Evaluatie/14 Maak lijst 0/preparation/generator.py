...
while len(cases) < ntests:
    ...
    nums = [0] + random.sample(range(1,40), k = math.floor(math.log10(n) * 10) )
    
    case = [random.choice(nums) for _ in range(n)]
...
for i in range(len(cases)):
    ...
    expression_name = f"minimaal_naar_nul({test})"
    ...
    try:
        ...
        with contextlib.redirect_stdout(outputF):
            result = module.minimaal_naar_nul(test)

...
while len(cases) < ntests:
    ...
    m = random.randint(n // 3, 4*n // 3)
    
    gulzigheid = [random.randint(1, 100) for _ in range(n)]
    koeken =[random.randint(1, 100) for _ in range(m)]
    
    
    case = (gulzigheid, koeken)
...
for i in range(len(cases)):
    ...
    expression_name = f"maximale_koeken({test[0]}, {test[1]})"
    ...
    try:
        ...
        with contextlib.redirect_stdout(outputF):
            result = module.maximale_koeken(test[0], test[1])

...
while len(cases) < ntests:
    ...
    som = random.randint(10**e, 10**(e+1))
    
    big_list = []
    for _ in range(3):
        lijst = []
        while sum(lijst) != som:
            getal = random.randint(sum(lijst) - som, som-sum(lijst))
            lijst.append(getal)
        big_list += lijst
    
    if random.randint(0,2) == 0:
        i, j = [random.randint(0, len(big_list)-1) for _ in range(2)]
        big_list[i] *= -1
        big_list[j] *= -1
    elif random.randint(0,2) == 1:
        i, j = [random.randint(0, len(big_list)-1) for _ in range(2)]
        a = big_list[i]
        big_list[i] = big_list[j]
        big_list[j] = a
...
for i in range(len(cases)):
    ...
    expression_name = f"partitie({test})"

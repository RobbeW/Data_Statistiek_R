...
while len(cases) < ntests:
    ...
    capaciteit = [random.randint(1, 100) for _ in range(n)]
    stenen = [ random.randint(0, capaciteit[i]) for i in range(n)]
    
    tekort = sum(capaciteit) - sum(stenen)
    
    extra = random.randint(1*tekort // 3, 4*tekort//3)
       
    
    case = (capaciteit, stenen, extra)
...
for i in range(len(cases)):
    ...
    expression_name = f"maximale_tassen({test[0]}, {test[1]}, {test[2]})"
    ...
    try:
        ...
        with contextlib.redirect_stdout(outputF):
            result = module.maximale_tassen(test[0], test[1], test[2])
        stdout = outputF.getvalue() ##printed value

        print(stdout)
        print(result)
        print("len", len(test[0]))

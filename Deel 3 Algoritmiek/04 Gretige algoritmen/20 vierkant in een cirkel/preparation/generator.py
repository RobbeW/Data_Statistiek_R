...
# generate test data
ntests = 25
cases = [([1, 2, 6, 7, 8], [2, 6, 7], [4, 8, 9])]

while len(cases) < ntests:
    N, M, K = [random.randint(1,100) for _ in range(3)]
    
    r_plot = [random.randint(1,100) for _ in range(N)]
    r_house = [random.randint(1,100) for _ in range(M)]
    z_house = [random.randint(1,100) for _ in range(K)]
           
    case = (r_plot, r_house, z_house)
    if case not in cases:
        cases.append(case)

cases = sorted(cases, key=lambda x: max(len(x[0]), len(x[1])+len(x[2])))
...
for i in range(len(cases)):
    ...
    expression_name = f"maximale_bebouwing({test[0]}, {test[1]}, {test[2]})"
    ...
    try:
        ...
        with contextlib.redirect_stdout(outputF):
            result = module.maximale_bebouwing(test[0], test[1], test[2])

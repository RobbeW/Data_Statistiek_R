...
while len(cases) < ntests:
    ...
    plan = [ random.randint(0,100) for _ in range(n)]
    
    k = e + random.randint(0,3)
    ships = random.randint(10**k, 10**(k+1))
...
for i in range(len(cases)):
    ...
    expression_name = f"ruimte({test[0]}, {test[1]})"
    ...
    try:
        ...
        with contextlib.redirect_stdout(outputF):
            result = module.ruimte(test[0], test[1])

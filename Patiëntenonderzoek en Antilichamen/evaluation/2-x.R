context({
  testcase('Check if patient_names variable is created correctly', {
    testEqual("length(patient_namen)", function(env) { length(env$patient_names) }, 5)
    testEqual("patient_namen[1]", function(env) { env$patient_names[1] }, "Alice")
    testEqual("patient_namen[2]", function(env) { env$patient_names[2] }, "Bob")
    testEqual("patient_namen[3]", function(env) { env$patient_names[3] }, "Karen")
    testEqual("patient_namen[4]", function(env) { env$patient_names[4] }, "David")
    testEqual("patient_namen[5]", function(env) { env$patient_names[5] }, "Eva")
  })

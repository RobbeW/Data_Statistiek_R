...
toename_sp_rb <- data$sp_post_rb - data$sp_pre_rb
toename_dp_rb <- data$dp_post_rb - data$dp_pre_rb

leeftijd_grote_stijging <- data$age[toename_sp_rb >= 10 & toename_dp_rb >= 10]

grote_stijging <- length(leeftijd_grote_stijging) / length(data$age) * 100

map_pre_rb <- round(data$dp_pre_rb + 1 / 3 * (data$sp_pre_rb - data$dp_pre_rb), 1)
map_post_rb <- round(data$dp_post_rb + 1 / 3 * (data$sp_post_rb - data$dp_post_rb), 1)